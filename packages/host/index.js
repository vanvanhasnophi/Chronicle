...
// Legacy compat: rewrites old /api/* paths to /api/admin/* or /api/public/*, then passes through
...
// Symlinks for dev static assets are created by start.sh.
// See: start.sh → manager/public/server/data/{upload,branding} → data/
...
if (!fs.existsSync(MANAGER_BACKGROUND_DIR)) fs.mkdirSync(MANAGER_BACKGROUND_DIR, { recursive: true });
...
function isEncryptedContent(raw) { return postService.isEncryptedContent(raw); }

function parseContentText(raw) { return postService.parseContentText(raw); }
...
function decodeHtmlEntities(text) { return postService.decodeHtmlEntities(text); }
...
function normalizeIndexPostRecord(post) { return postService.normalizeIndexPostRecord(post); }
...
function stripHtml(text) { return postService.stripHtml(text); }

function injectIdsIntoHtml(html) { return postService.injectIdsIntoHtml(html); }
function slugifyHeading(text) { return postService.slugifyHeading(text); }
...
function syncIndexWithFiles() { return postService.syncIndexWithFiles(); }
syncIndexWithFiles();
...
app.use('/server/data/branding', express.static(BRANDING_DIR, {
    maxAge: '7d',
    immutable: true
}));
app.use('/server/data/manager-background', express.static(MANAGER_BACKGROUND_DIR, {
    maxAge: '7d',
    immutable: true
}));
