/**
 * Chronicle Marp Theme — shared CSS generators (light + dark)
 *
 * Used by both:
 *   - manager (CMS preview)  →  chronicleThemes.ts re-exports
 *   - template-astro (SSG)   →  Slideshow.astro registers with marp-core
 *
 * Theme names registered with Marp:
 *   - `chronicle`       →  light theme
 *   - `chronicle-dark`  →  dark theme
 */
...
section code{color:#111;font-family:"Consolas","JetBrains Mono","Fira Code",monospace;background:#f0f0f0;padding:2px 6px;border-radius:4px}
...
.mermaid-prerendered .label,.mermaid-prerendered .nodeLabel,.mermaid-prerendered .messageText,.mermaid-prerendered .legend,.mermaid-prerendered .edgeLabel,.mermaid-prerendered span,.mermaid-prerendered text,.mermaid-prerendered p{background:${bg}!important;color:#111!important}
...
.mermaid-prerendered svg line,.mermaid-prerendered svg rect,.mermaid-prerendered svg polygon,.mermaid-prerendered svg circle,.mermaid-prerendered svg triangle,.mermaid-prerendered svg path:not([class]){fill:${bg}!important;stroke:#111!important}
...
section p{margin:0 0 .5em 0}
section a{color:${accent}}
section code{color:#eee;font-family:"Consolas","JetBrains Mono","Fira Code",monospace;background:#161616;padding:2px 6px;border-radius:4px}
...
.mermaid-prerendered svg{max-width:100%;height:auto}
.mermaid-prerendered .label,.mermaid-prerendered .nodeLabel,.mermaid-prerendered .messageText,.mermaid-prerendered .legend,.mermaid-prerendered .edgeLabel,.mermaid-prerendered span,.mermaid-prerendered text,.mermaid-prerendered p{background:${bg}!important;color:#eee!important}
.mermaid-prerendered svg,.mermaid-prerendered svg *{font-family:inherit!important}
...
.mermaid-prerendered svg line,.mermaid-prerendered svg rect,.mermaid-prerendered svg polygon,.mermaid-prerendered svg circle,.mermaid-prerendered svg triangle,.mermaid-prerendered svg path:not([class]){fill:${bg}!important;stroke:#eee!important}
.mermaid-prerendered svg marker path{fill:#eee!important;stroke:#eee!important;stroke-width:1.5!important}
@media print{.no-print{display:none!important}}`
}

/** Default theme CSS strings (default accent) */
export const chronicleCSS = light()
export const chronicleDarkCSSStr = dark()

/** Generate Chronicle light theme CSS with custom accent. */
export function chronicleLightCSS(accent: string, tintedBg = false) { return light(accent, tintedBg) }
/** Generate Chronicle dark theme CSS with custom accent. */
export function chronicleDarkCSS(accent: string, tintedBg = false) { return dark(accent, tintedBg) }
