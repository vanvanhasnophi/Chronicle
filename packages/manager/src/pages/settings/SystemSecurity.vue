        ...
        <div v-if="passkeys.length > 0" class="passkey-list">
          ...
          <div v-for="pk in passkeys" :key="pk.id" class="passkey-item">
            <div class="pk-info">
              ...
              <span class="pk-date">{{ $t('security.added') }}: {{ formatDateUtil(String(pk.createdAt), getLocaleStr())
                }}</span>
            </div>
            <div class="pk-actions">
              <button class="icon-btn edit-btn" @click="renamePasskey(pk.id, pk.name || $t('security.unnamedKey'))"
                :title="$t('security.rename')" v-html="Icons.edit"></button>
              <button class="icon-btn delete-btn" @click="deletePasskey(pk.id)" :title="$t('security.delete')"
