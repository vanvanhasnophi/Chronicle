...
- **`file:///` URLs with spaces/CJK not encoded**: `fileToUrl` now passes paths through `encodeURI` (spaces → `%20`, CJK → UTF-8 hex). `insertMediaMarkdown` applies `encodeURI(decodeURI(...))` round-trip to all markdown URLs — handles unencoded spaces/CJK without double-encoding. `FileManager.copyLink` does the same.
...
- **`resolveLocalFileUrls` prefix regex ate `file:` from `file:///` — images only**: the regex `/^(audio|video|document|text|file):/` matched `file:` at the start of `file:///path/photo.jpg`, stripping it to `///path/photo.jpg` and causing silent skip. Non-images were shielded by their type prefix (`audio:file:///...` → regex strips `audio:`, leaving valid `file:///`). Fixed by removing `file` from the regex and handling it separately with a `(?=blob:|file:\/\/\/)` lookahead.
