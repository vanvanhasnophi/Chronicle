...
const fileTypeTabs = [
    { id: 'all', label: t('file.categories.all') },
    { id: 'image', label: t('file.categories.images') },
    { id: 'video', label: t('file.categories.videos') },
    { id: 'audio', label: t('file.categories.audio') },
    { id: 'document', label: t('file.categories.documents') },
    { id: 'text', label: t('file.categories.text') },
    { id: 'other', label: t('file.categories.others') },
]
...
function detectTypeTab(file: any) {
    const { isImage, isVideo, isAudio, isDocument } = detectFileType(file)
...
const getFileType = (name: string) => {
    const { isAudio, isVideo, isDocument, isCode } = detectFileType(name)
...
function refreshVisibleFiles() {
    visibleFiles.value = activeTypeTab.value === 'all'
...
async function loadCloudFiles() {
    ...
    try {
        const res = await fetchWithAuth(`/api/files?path=all&t=${Date.now()}`, { cache: 'no-store' } as any)
...
function triggerLocalPicker() {
    if ((window as any).showOpenFilePicker) {
        ; (window as any).showOpenFilePicker({ multiple: allowMultiple }).then(async (handles: any[]) => {
